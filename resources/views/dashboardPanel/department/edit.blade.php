<x-app-layout>
  ...
  <div class="flex shadow  sm:gap-0 bg-white mx-auto  max-w-2xl justify-center   p-4">
    <form action="/categories/{{$category->id}}" class="flex flex-col gap-4 w-full" method="post">
      @csrf
      @method('patch')
      <div class="flex flex-col gap-2 mb-4 w-full">
        <label for="name " class="text-gray-600">category name</label>
        <input class="w-full" id="name" type="text" name="name" value="{{$category->name}}" required class="border border-gray-200">
        @error('name')
        <p>{{$message}}</p>
        @enderror
      </div>
